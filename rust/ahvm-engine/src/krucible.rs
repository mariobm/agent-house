...
use crate::{
    host_caps, send_ctl, spawn_worker_cfg, Backend, BackendKind, Capabilities, Error, ExecResult,
    Result, SandboxInfo, SandboxSpec, SnapshotManifest, SpawnConfig, State, Thermal, Worker,
    MAX_EXEC_OUTPUT,
};
use crate::{is_alive, terminate_adopted, LiveWorker};
...
/// One forge RPC round trip (mirrors the KVM gate's 15s socket timeouts).
...
impl WorkerHandle {
    ...
    fn alive(&mut self) -> bool {
        match self {
            ...
            WorkerHandle::Adopted(w) => is_alive(w.pid),
        }
    }

    /// Stop and clean up. Owned: SIGKILL + reap (no zombie, idempotent).
    /// Adopted: SIGTERM + bounded pid poll (no handle to wait on).
    ...
    pub fn open(cfg: KrucibleConfig) -> Result<Self> {
        ...
        for dir in dirs {
            ...
            let worker = match Worker::load(dir.join("state.json")) {
                Ok(w) if is_alive(w.pid) => Some(WorkerHandle::Adopted(w)),
                _ => None,
            };
...
fn connect_rpc(sock: &Path, deadline: Instant) -> Result<UnixStream> {
    loop {
...
fn rpc_exec(sock: &Path, argv: &[String], timeout: Duration) -> Result<ExecResult> {
    let deadline = Instant::now() + timeout;
    let mut c = connect_rpc(sock, deadline)?;
...
impl KrucibleBackend {
    ...
    /// Online snapshot: PAUSE, SNAPSHOT, freeze the overlay, write the
    /// sidecar, RESUME. The VM stays live (fork/create_snapshot path).
    ...
    fn snapshot_live(&self, dir: &Path, rec: &SandboxRecord, snapshot_id: &str) -> Result<PathBuf> {
        ...
        let snap = (|| -> Result<PathBuf> {
            let reply = send_ctl(&ctl, &format!("SNAPSHOT {}", bundle.display()))?;
            if !reply.starts_with("OK") {
            ...
            let frozen = bundle.join("root.qcow2");
            let root_bytes = std::fs::copy(dir.join("root.qcow2"), &frozen)?;
            if root_bytes == 0 {
...
impl Backend for KrucibleBackend {
    ...
    fn start(&self, id: &str) -> Result<()> {
        ...
        let plan = {
            ...
            if rec.record.info.state == State::Running {
                Plan::AlreadyRunning
            } else if rec.dir.join("bundle").join("manifest.json").is_file() {
                Plan::Restore {
                    dir: rec.dir.clone(),
    ...
    fn exec(&self, id: &str, argv: &[String]) -> Result<ExecResult> {
        ...
        rpc_exec(&forge_sock(&dir), argv, self.cfg.ready_timeout)
    }
    ...
    fn create_snapshot(&self, id: &str, snapshot_id: &str) -> Result<SnapshotManifest> {
        ...
        copy_dir(&bundle, &reg)?;
        let manifest = SnapshotManifest::read_from(&reg)?;
        self.lock()
            .snapshots
    ...
    fn restore(&self, snapshot: &SnapshotManifest, new_id: &str) -> Result<SandboxInfo> {
        validate_id(new_id)?;
        snapshot.check_compat(&self.host_caps())?;
        let mut inner = self.lock();
        ...
        // Spec derives from the manifest (like the mock): sizing is part of
        // the compat gate, so a passing gate implies a bootable shape.
    ...
    fn fork(&self, id: &str, new_id: &str) -> Result<SandboxInfo> {
        ...
        let manifest = self.create_snapshot(id, &internal)?;
        let mut info = self.restore(&manifest, new_id)?;
...
mod tests {
    ...
    fn ids_and_snapshot_ids_are_validated_without_touching_disk() {
        ...
        // Compat gate fires before any worker spawns: register a tampered
        // bundle dir by hand, then restore must refuse it as incompatible.
        let reg = dir.join("data").join("snapshots").join("tampered");
        let mut bad = snap.clone();
        bad.snapshot_id = "tampered".to_string();
        bad.compat.vmm = VmmId {
            name: "firecracker".to_string(),
            version: "1.0".to_string(),
        };
        bad.write_to(&reg).unwrap();
        let be = KrucibleBackend::open(cfg(&dir)).unwrap();
        assert!(matches!(
            be.restore(&bad, "new2"),
    ...
    fn open_adopts_live_pids_and_marks_dead_failed() {
        ...
        // Live record: our own test pid is alive by definition.
