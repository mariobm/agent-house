# krucible — the guest init model (forge-as-PID-1 vs init-mediated)

Status: **Design doc (2026-06-16).** Scopes what the "forge → service" restructure concretely takes, after the cold-tier
work surfaced friction between ahvm's `forge-as-PID-1` model and libkrun's init-centric design. Companion:
`docs/PLAN-krucible-cold-tier.md` (the cold tier + the block-root-vs-FUSE decision), `docs/internal/PLAN-krucible-v3.md` (P1's
"forge is the asset" decision), `docs/archive/v1/guest-agent.md` (retired; canonical: <https://ahvm.sh/docs/under-the-hood/forge-the-blacksmith/>).
...
## DECISION (2026-06-16): we shipped M1′ (kernel-direct block root), and the forge slim is moot
...
- **M1 (as planned):** libkrun's bundled **init** (PID 1) mounts `/dev/vda`, pivots, then `execvp`s forge → forge could
...
- **M1′ (what we shipped):** no init-blob. The kernel cmdline carries `root=/dev/vda rootfstype=ext4 ... init=/init.krun`,
  so the **kernel** mounts the ext4 root and execs forge directly as PID 1. The bundled kernel has virtio-blk+ext4 built
  in (`nomodule`). forge keeps **every** PID-1 duty — `/proc`,`/sys`,`devtmpfs`,`devpts`,`tmpfs`×3,`cgroup2`,`binfmt_misc`,
  loopback, signals/reboot, reaping, vsock listeners — because the kernel only mounts the *root*, nothing else.

**Consequence — the envisioned forge slim does not apply:** there is no libkrun init to hand early boot to, so forge's
mount/pivot/PID-1 block stays as-is. We chose this deliberately: **avoiding the init-blob cross-compile toolchain is worth
more than the marginal forge reduction.** The cold tier is green with forge unchanged.

**The one piece of FC-era code in forge (`setupNetworking`, the `ip=`/TAP path in `net.go`) is *not* deletable and needs
...
**Net: forge is already appropriately lean for the krucible path.** No slimming action is taken. The real forge
...
The framing "demote forge to a non-PID-1 *service*" **does not fit what forge is.** forge is not just an agent — it is
...
distro init does service management.* ahvm deliberately went the other way: **forge *is* the service manager**, so it is
...
- **The realistic, bounded restructure is B1: keep forge as PID 1, but let libkrun's init do the *early* boot** (mount the
  block root + pivot + base mounts), then `exec` forge. This unlocks the **block root** (→ cold-tier exec-after-restore,
...
### M0 — today: `disable_implicit_init`, forge = PID 1, virtio-fs root
Kernel `init=/init.krun`, `/init.krun` = forge. forge does *everything* (mounts → service-manager → agent). libkrun's own
...
### M1 (B1) — init-mediated boot, forge still PID 1, block root
Do **not** `disable_implicit_init`. `krun_set_root_disk(<ext4 image>)` + `krun_set_exec(<forge>)`. libkrun's init (PID 1)
mounts `/dev/vda`, pivots to it, then `execvp`s forge — which **becomes PID 1** (exec replaces the init) and continues as
...
- **Wins:** block root boots → cold-tier exec-after-restore works (block persist already done); guest confined to the image
  ...
  self-contained/CoW-overlay snapshots. forge sheds *early-boot* duty (mount/pivot) but keeps service-manager + agent.
- **Cost:** forge's remaining mounts must become **idempotent** (libkrun's init already mounted `/proc`,`/sys`,`/dev` and
...
### M2 (B2) — real init/supervisor = PID 1, forge = service
libkrun init → a minimal supervisor (PID 1: reap + spawn + restart + reboot) → forge as a **child service**, possibly
...
- **Blocker:** *who runs the systemd shim?* It is the service manager and is inherently PID 1. M2 only makes sense if we
  ...
  child — which is a real forge split (agent ↔ service-manager) and a larger refactor.
...
- **Stop calling `krun_disable_implicit_init`** on the cold/fork-capable profile. Set `krun_set_root_disk(<image>)` +
  `krun_set_exec(<forge path on the image>)`. libkrun's init mounts + pivots + execs forge. (Already wired:
...
### forge (the bounded guest changes)
- **Idempotent early boot.** `mustMount(...)` must become "skip if already a mountpoint" (check `/proc/mounts`), because
  ...
  re-mounts. (forge already has mountpoint-detection logic to reuse.)
- **Relax / re-scope the PID-1 refuse-guard** carefully: forge is *still* PID 1 under M1 (exec'd by the init), so the
  `getpid()==1` guard still holds — good. No change to the guard's intent.
- **Boot-source detection:** forge should not assume virtio-fs (`no ip= … skipping` etc. stays); with a block root the
...
- **What stays untouched:** the entire systemd shim, the agent protocol (Exec/Shell/Files/Sessions/Tunnel), zombie
  reaping, signals, syslog. forge remains the init + service-manager + agent.
...
| Risk | Mitigation |
|---|---|
| Double-mount / EBUSY when both libkrun-init and forge mount | Make forge mounts idempotent (check `/proc/mounts`); test the exact post-pivot mount set |
| PID-1 boot regression (the W4 bricked-Pi class) | Keep the `getpid()==1` guard; gate behind a profile; heavy `FORGE_TEST=1` + VM smoke before default |
| libkrun init's exec/env contract differs from our config-drive | forge keeps reading the config drive (separate disk); the libkrun-init cmdline env is parallel, not a replacement |
...
3. **Run the agent as a child service**, ideally as a dropped-privilege uid with a capability set (the agent does need
   ...
   supervisor via a tiny privileged IPC, mirroring forge's existing `systemctl_ipc` split).
4. **Re-home the PID-1 duties** forge does today (mounts, reaping, reboot) into the supervisor.

This is a genuine guest re-architecture with its own test surface (the agent's ~40-func `FORGE_TEST=1` suite assumes the
...
- **Security:** libkrun states virtio-fs has *no* protection against the guest reaching other host dirs/filesystems
  ...
  syscalls). Strictly better confinement; M1 gets this even while forge stays PID 1.
...
The agent-as-service *robustness/least-privilege* wins (no-panic, dropped privileges) are **B2-only** — M1 keeps forge PID
...
1. **Now (finish P3): M1.** Init-mediated **block root**, forge stays PID 1. Bounded forge change (idempotent mounts) +
   engine image/overlay build. Delivers exec-after-restore on the already-built block persist, plus the security/perf/
   snapshot wins of a block root. Lowest risk to the validated cold-wake and to forge's contract.
2. **Decision gate (P4): the systemd shim on krucible.** Agent-first only, or Docker/systemd workloads too? This answer
   ...
   agent↔service-manager split.
3. **Then (if M2 chosen): the forge split** on its own gate, with the agent's test suite reworked for the unfused model.
...
## 6. The real question: forge's *delta* in libkrun (what's better NOT done in forge)

Much of forge exists because **Firecracker gave us nothing** — FC boots a kernel and runs `init=`, full stop. So forge had
to *be* the init, the network setup, the clock-workaround, the service manager, and the agent. libkrun is the opposite: it
ships an init and a device/boot model, and our own VMM work (the warm/cold tier) added the missing pieces. A lot of forge
is therefore **FC-era workaround that libkrun now subsumes.** The discussion isn't "shrink forge for its own sake" — it's:
*where libkrun (or the VMM) does a job natively and better, forge carrying its own version is liability, not value.*

### Why forge did things its own way (documented) — and what changed
- **No systemd (decisions §3):** determinism + boot speed (367ms vs 708ms) **and snapshot-correctness** — real systemd
  ...
  `degraded`/`maintenance` mode (`EXPERIMENT-systemd-predictions.md`). forge-as-PID-1 has no timers/watchdogs to misbehave.
...
- **Own networking (`net.go`, `ip=`):** FC used TAP + kernel `ip=`. **What changed:** TSI — no `eth0`; forge's network
  setup already no-ops on krucible. Dead code on this path.

### Responsibility inventory: forge today → libkrun-native → verdict
| forge does today | libkrun / VMM native? | Verdict on krucible |
|---|---|---|
...
### Where the value is
Strip the FC-era plumbing and forge's *unique, defensible* value is two things, of very different natures:
...
- **init plumbing → libkrun's init** (M1): mounts, pivot, net, reaping, reboot, redirects. Delete forge's copies.
- **clock workarounds → gone**: the VMM owns clock continuity. (Re-audit forge/the shim for any monotonic-jump
  defensiveness that's now redundant.)
- **agent → the thin core of forge**, the workload the init execs (M1) — or a service (M2) once supervision is settled.
...
Net: libkrun doesn't make forge *less* valuable — it lets forge **stop pretending to be an OS** and be the thing only
...
3. **Idempotent-mount surface in forge:** exact post-libkrun-init mount set to skip; any ordering assumptions (cgroup2,
   binfmt_misc) that the libkrun init doesn't establish and forge must still do.
