...
**Decision:** Forge listens on both vsock and TCP on the same ports (1024/1025). Cold boot uses vsock (slightly faster initial connection). After restore, a new `AgentClient` is created that uses TCP. The agent doesn't need to know — it accepts connections from either transport.
...
**Decision:** Forge *is* PID 1. The kernel boots directly into it via `init=/usr/local/bin/forge`. No systemd, no initramfs, no init scripts.
...
**Tradeoff:** Forge must handle everything PID 1 is responsible for: mounting filesystems, signal handling, and (in theory) reaping orphan zombies. The zombie reaping is intentionally omitted — Go's runtime manages `SIGCHLD` for `exec.Command` processes, and a manual `Wait4(-1)` reaper would race with `cmd.Wait()`. Orphan zombies are acceptable because the VM is short-lived.
...
**Decision:** Forge writes to a temp file (`path.ahvm-tmp`), fsyncs it, then renames it over the target atomically:
...
**Why kernel `ip=`:** The guest network must be up before forge starts, because the host polls forge via TCP to detect readiness. If forge configures networking, the host can't reach forge to tell it what IP to use. The kernel `ip=` parameter is processed during early boot — the interface is configured before init runs. Zero DHCP, zero latency, zero failure modes.
