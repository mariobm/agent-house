# Fix the cgroup-placement race with a `forge spawn` helper

Status: shipped in v1.11.10 (commits 623e8ad...e60035a).
Target release: `v1.11.10` (patch — bug fix, no API/UX changes).
Touches: `cmd/forge/main.go` (~5 lines for argv dispatch) + `cmd/forge/spawn.go` (new, ~60 lines) + `cmd/forge/systemctl.go` (refactor `startDaemon`'s spawn call, ~10 lines net) + `cmd/forge/spawn_test.go` (new, ~120 lines) + `cmd/forge/cgroup_test.go` (one new assertion).
...
The shim's `startDaemon` (`cmd/forge/systemctl.go`) acknowledges this exact failure mode:
...
- A new `forge spawn` subcommand whose entire job is "enter this cgroup, then `execve` into this argv". One file, no business logic.
- `cmd/forge/main.go`: extend the argv-based dispatch (already used for `systemctl`/`journalctl`) to recognise `spawn` as a verb.
- `cmd/forge/systemctl.go`: replace the `exec.Command("/bin/sh", "-c", "exec "+execStart)` + post-spawn `cgroup.procs` write with a single `exec.Command("/proc/self/exe", "spawn", "--cgroup", path, "--", "/bin/sh", "-c", "exec "+execStart)`. The post-spawn `PlaceInCgroup` call is removed outright (not left as a "defensive copy") — two writers to the same kernel file means two places that can drift and a fuzzier contract for `forge spawn`. The new integration test (below) is the safety net.
- Tests: a `cmd/forge/spawn_test.go` covering the happy path, the cgroup-write failure path, and the argv-parse edge cases; one new assertion in `cgroup_test.go` (or a new `TestStartDaemonPlacesIntoUnitCgroup`) that reads `/proc/<pid>/cgroup` after a fresh start and confirms it ends in `<unit>.service`, not `0::/`.
...
| Failure | Behaviour |
|---|---|
| Cgroup directory doesn't exist (forge's `CreateCgroup` didn't run or failed silently) | `os.WriteFile` returns `ENOENT`. `forge spawn` prints `cgroup.procs: <err>` to stderr and `os.Exit(1)`. No daemon is started — clean failure visible in `journalctl -u X` as start error. |
| Permission denied on cgroup write | Same as above; `EACCES`. Caller sees non-zero exit code from `cmd.Wait()`. |
| Argv after `--` is malformed | `forge spawn` exits 2 with usage. Hard error before any state change. |
...
| `forge spawn` is killed between cgroup write and exec | Daemon never starts. Pidfile points at the dead forge-spawn PID; `IsRunning` returns false on next poll; watcher's Restart policy kicks in (which re-spawns; same race we want to avoid is gone because each spawn is its own atomic placement). |
...
**B. Walk `/proc` on stop and kill descendants of the pidfile PID.** Doesn't fix the resource-accounting half of the bug (the orphan was in the wrong cgroup *all along*, not just at stop-time). Misses double-fork: when X-server's detach forks and the parent exits, the child gets reparented to forge (PID 1, subreaper) and the PPid chain is lost. Plus all the usual `/proc` walk pitfalls (PID reuse, races with new processes during the walk). Rejected.
...
### Unit tests in `cmd/forge/spawn_test.go`
...
### Integration test in `cmd/forge/cgroup_test.go` (or a new `start_daemon_test.go`)
...
```bash
# 1. Build a fresh computer-tier rootfs locally using the new forge.
sudo ./scripts/build-tier.sh computer amd64 ./bin/forge-linux-amd64
...
If all green: tag, release. CI rebuilds rootfs (no rootfs change is needed for this fix — just the kernel-cmdline `init=/usr/local/bin/forge` automatically loads the new binary; existing units don't need to change).
...
### `docs/under-the-hood/forge-the-blacksmith.mdx`
...
- The two-process spawn chain (`forge spawn` then the daemon), with a short diagram similar to the one in this plan.
...
- The choice (`forge spawn`) and why over `clone3` (Go runtime fork-safety surgery vs. ~60 LOC of plain Go; same correctness; lower maintenance surface; matches the busybox-pattern dispatch we already use for `systemctl` and `journalctl`).
...
2. **Tighten the "common operator story" paragraph** to say explicitly that the model is uniform across tiers ("Every long-running process in every built-in tier is managed by forge's systemctl shim. The same three commands — `systemctl status`, `journalctl -u`, `systemctl restart` — work the same way regardless of which tier you started from."). The current text gets close; this nails it.
...
- The user-tunable knob convention: `Environment=` for defaults, `EnvironmentFile=-/run/ahvm/config-env` so `ahvm create --env KEY=value` overrides take effect. Reference the existing config-env bridge section in forge internals.
...
- A note that tier authors don't need to know about `forge spawn` directly — their unit's ExecStart is what they care about; the spawn helper is upstream of that, invisible to the unit file.
...
| Risk | Mitigation |
|---|---|
| `forge spawn` writes its PID to `cgroup.procs` but then fails to find the daemon binary (typo in ExecStart) — pidfile points at a dead forge-spawn, watcher restart loops forever | Same behaviour as today's "ExecStart points at a missing binary": watcher sees rapid exits, hits `restart-burst` limit, gives up. Marked as failed in `is-failed`. The user sees a meaningful error in `journalctl -u` because `forge spawn` writes its own startup failures there before `execve` succeeds. |
| The cgroup directory is removed between `CreateCgroup` and `forge spawn` running | `os.WriteFile` returns `ENOENT`, spawn exits 1, supervisor reports failure. We don't currently delete cgroup dirs mid-spawn so this is theoretical; if it ever does happen the failure mode is clean. |
| Pidfile reads wrong because `cmd.Process.Pid` is set before the cgroup write | `cmd.Process.Pid` IS the PID we want in the pidfile — it's the PID of `forge spawn`, which is preserved across the two `execve`s and ends up being the daemon's PID. No change to pidfile semantics. |
...
| `User=`/`Group=` drop interacts badly with `cgroup.procs` write | Not currently triggered — the shim doesn't honour `User=`/`Group=` at all today (no `Credential` is set on the outer `exec.Command`, the directive isn't parsed). Tracked future state: when a tier needs it, the drop lives inside `forge spawn` after the `cgroup.procs` write and before the `syscall.Exec`. It must not be set on the outer `exec.Command` because `cgroup.procs` is `0644` root-owned and the dropped uid cannot write it. (Pre-writing the PID from the supervisor before `cmd.Start()` is the other alternative; it resurrects the race we just fixed, so it's a non-option.) |
...
1. **`forge spawn` subcommand.** New file `cmd/forge/spawn.go` plus the argv dispatch hook in `main.go`. Includes the new `spawn_test.go`. Reviewable as a self-contained piece — does one thing, has no callers yet.

2. **Wire the supervisor through it.** Change `startDaemon` in `cmd/forge/systemctl.go` to invoke `forge spawn` via `/proc/self/exe`. **Remove the post-spawn `PlaceInCgroup` call outright** — no defensive copy. Two writers to the same kernel file means two places that can drift and a fuzzier contract for `forge spawn`. The whole point of the helper is to make the write happen at a known correctness checkpoint; the new test is the safety net. Add `TestStartDaemonPlacesProcessInUnitCgroup` to `cgroup_test.go`.

3. **Smoke on agni-01.** Run the integration test plan above. If a real bug shows up here, fix it; don't ship a rotted v1.11.10.

4. **Tag `v1.11.10`.** CI builds binaries + rootfs (unchanged tier scripts, but rootfs rebuilds anyway so the new forge gets baked in).
...
6. **`ahvm.sh` docs PR** (separate repo, separate PR). Four pages touched, in one commit, all paths under `src/content/docs/docs/`:
   - `under-the-hood/forge-the-blacksmith.mdx` — new "How services are spawned" section + reframed "What the shim doesn't do" footnote.
...
5. **The two-`execve` chain (forge → sh → daemon) is preserved across both transitions.** `forge spawn` does `syscall.Exec("/bin/sh", ...)`, then sh's built-in `exec` keyword does its own `execve` into the daemon. Both are real kernel-level execves, both preserve PID and cgroup. The "sh as intermediary" is the same shape as today; the only thing we're adding is "forge as intermediary" in front of it. No semantic shift.
