...
# command's stdin (for `ahvm file write`, which reads content from stdin).
time_ms_in() {
  perl -MTime::HiRes=time -e '
    my $in = shift @ARGV;
    my $t0 = time;
...
"$REPO/ahvm" serve > "$WORK/serve.log" 2>&1 &
SRV=$!
cleanup() {
  # destroy any leftover sandboxes, then stop the daemon
  for s in $("$REPO/ahvm" list 2>/dev/null | awk 'NR>1 {print $1}'); do
    "$REPO/ahvm" destroy "$s" --yes >/dev/null 2>&1 || true
  done
  kill "$SRV" 2>/dev/null || true
  sleep 0.5
  # backstop: kill any helper still bound to this run's data dir
  pkill -f "ahvm-vmm $WORK/data" 2>/dev/null || true
}
trap cleanup EXIT
...
for i in $(seq 1 "$N_LIFECYCLE"); do
  ...
  t=$(time_ms "$REPO/ahvm" create --name "b-create-$i" --cpus 1 --memory 512) && echo "$t" >> "$f"
done
summarize "$f"
...
for spec in "true:/bin/true" "echo:/bin/echo hi" "sha:sha256sum /etc/os-release"; do
  ...
  for i in $(seq 1 "$N_EXEC"); do
    t=$(time_ms "$REPO/ahvm" exec bench-sb -- $cmd) && echo "$t" >> "$f"
  done
...
for sz in 1024:1k 102400:100k 1048576:1m; do
  ...
  for i in $(seq 1 "$N_FILE"); do
    t=$(time_ms_in "$tmp" "$REPO/ahvm" file write bench-sb "/tmp/f-$label") && echo "$t" >> "$fw"
    t=$(time_ms "$REPO/ahvm" file read bench-sb "/tmp/f-$label") && echo "$t" >> "$fr"
  done
...
for i in $(seq 1 "$N_COLD"); do
  ...
  t=$(time_ms "$REPO/ahvm" stop bench-sb) && echo "$t" >> "$fs"
  t=$(time_ms "$REPO/ahvm" start bench-sb) && echo "$t" >> "$fr"
  ...
  ' -- "$REPO/ahvm" start bench-sb "$REPO/ahvm" exec bench-sb -- /bin/true) && echo "$t" >> "$fe"
done
...
for c in 5 10 20; do
  ...
  for r in $(seq 1 "$CONCURRENCY_REPS"); do
    ...
    ' "$c" "$REPO/ahvm" exec bench-sb -- /bin/true) && echo "$t" >> "$f"
  done
...
for i in $(seq 1 "$N_LIFECYCLE"); do
  t=$(time_ms "$REPO/ahvm" destroy "b-create-$i" --yes) && echo "$t" >> "$f"
done
summarize "$f"
