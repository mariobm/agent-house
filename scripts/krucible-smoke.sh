...
# forge as PID 1, boot it, and prove the agent answers over the bridged vsock.
#
...
# --- minimal rootfs: forge @ /init.krun (PID 1) + a tiny /bin/true for exec ---
echo "==> build rootfs (forge @ /init.krun)"
ROOT="$WORK/rootfs"
rm -rf "$ROOT"
mkdir -p "$ROOT"/{usr/local/bin,bin,proc,sys,dev/pts,tmp,run,etc,root}
GOOS=linux GOARCH="$GUEST_ARCH" CGO_ENABLED=0 go build -o "$ROOT/init.krun" ./cmd/forge
...
if "$WORK/krucible-probe" "$UDS"; then
  echo ""
  echo "SMOKE PASS: forge booted as PID 1 and the agent answered over vsock."
  echo "  (boot log: grep forge $WORK/vmm.log)"
else
