...
#
# forge runs as PID 1 and provides a systemctl shim (busybox pattern).
...
# systemctl + journalctl shims — forge handles these via busybox pattern.
# Package postinst scripts call systemctl enable/start/stop; these symlinks
# route those calls to forge's built-in service manager.
ln -sf /usr/local/bin/forge "$MOUNT/usr/bin/systemctl"
ln -sf /usr/local/bin/forge "$MOUNT/usr/bin/journalctl"
