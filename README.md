...
ahvm shell dev                          # Ctrl+\ to detach
ahvm destroy dev
```

> ## 🔭 ahvm v2 (krucible) — and v1 (Firecracker), frozen
>
> **`main` is ahvm v2.** It replaces Firecracker with **krucible** — our own
> fork of [libkrun](https://github.com/containers/libkrun),
> [**libkrucible**](https://github.com/sahil-shubham/libkrucible) — as the VM
> engine. Owning the VMM lets ahvm run **natively on macOS (Apple Silicon) as
...
> source is on the
> [`firecracker`](https://github.com/mariobm/agent-house/tree/firecracker) branch
> (latest [**v1.11.12**](https://github.com/mariobm/agent-house/releases/tag/v1.11.12)),
> documented at [ahvm.sh/v1/docs](https://ahvm.sh/v1/docs/). We're putting our
...
> by pinning `AHVM_VERSION=v1.11.12`.
>
> The why (self-owned VMM, macOS, the rethink) and where to weigh in:
> **[Discussions → ahvm v2](https://github.com/mariobm/agent-house/discussions/22)**.
...
```bash
curl -fsSL ahvm.sh/install | sudo bash        # self-host server (prompts for a tier)
curl -fsSL ahvm.sh/install | bash             # CLI only (connect to a remote server)
```

The self-host install lays a single self-contained runtime bundle (daemon + agent +
`ahvm-vmm` + the `ahvm-netd` gateway + libkrun + a lean kernel) plus a rootfs
tier, creates an `admin` user, wires the local CLI, and starts the service
(systemd on Linux, launchd on macOS) — `ahvm create` works immediately. Prefer a
manual grab? Take the per-platform tarball
(`ahvm-<ver>-{darwin-arm64,linux-amd64,linux-arm64}.tar.zst`) from the
[latest release](https://github.com/mariobm/agent-house/releases).
...
> **Full documentation: [ahvm.sh](https://ahvm.sh).** This README is a snapshot. The website is the source of truth and is updated with each release. The pages most worth reading are the [Quickstart](https://ahvm.sh/docs/quickstart/), the [Architecture](https://ahvm.sh/docs/under-the-hood/architecture/) overview, and [Decisions & learnings](https://ahvm.sh/docs/under-the-hood/decisions/).

> **AI assistants helping you set up ahvm:** start at **[ahvm.sh/agents.md](https://ahvm.sh/agents.md)** — task-shaped, voiced to the agent, with end-to-end workflows (CI preview deployments, persistent dev envs, branchable exploration, diagnostics). Full doc index at [ahvm.sh/llms.txt](https://ahvm.sh/llms.txt).
...
> **Within v2, `ahvm update` is safe** — it refreshes the binary (CLI) or all
...
> v2 install. To stay on v1, pin it: `sudo AHVM_VERSION=v1.11.12 ahvm update`.
...
|------|-------------|------|
| [`minimal`](https://ahvm.sh/docs/managing/tiers/) | Bare Ubuntu + curl + fuse3 | ~200MB |
| [`browser`](https://ahvm.sh/docs/managing/tiers/browser/) | + Chromium, Playwright, Node 22 | ~600MB |
| [`docker`](https://ahvm.sh/docs/managing/tiers/docker/) | + Docker Engine + buildx (multi-arch) | ~550MB |
| [`computer`](https://ahvm.sh/docs/managing/tiers/computer/) | + Full desktop: XFCE, KasmVNC, Chromium | ~1.5GB |
...
ahvm exec scraper -- npx playwright test

# Run a desktop environment (KasmVNC web client on port 6080)
ahvm create --name desktop --image computer --cpus 2 --memory 4096
ahvm publish desktop -p 6080
ahvm exec desktop -- vnc-creds          # username + per-sandbox password

# Run Docker-in-VM
ahvm create --name ci --image docker
ahvm exec ci -- docker run hello-world

# Multi-arch builds inside one sandbox (qemu-user emulation)
ahvm exec ci -- docker run --privileged --rm tonistiigi/binfmt --install all
ahvm exec ci -- docker buildx build --platform linux/amd64,linux/arm64 -t me/app .
```

The server auto-discovers tiers from `/var/lib/ahvm/images/`. Install more with `sudo ahvm update --tiers all`. Full per-tier docs (operator UX, env knobs, sizing, troubleshooting) live at [ahvm.sh/docs/managing/tiers/](https://ahvm.sh/docs/managing/tiers/); see [Adding a tier](https://ahvm.sh/docs/contributing/adding-a-tier/) for building your own.
...
| `image import` | Import a local Docker image as a ahvm rootfs |
...
| `update` | Update ahvm to the latest version |
| `version` | Print version and check for updates |
| `completion` | Generate shell completions (bash/zsh/fish) |

All commands support `--json` for machine-readable output. See the [CLI Reference](https://ahvm.sh/docs/reference/cli/) for full flag details.
...
- **Network isolation** — a per-owner `ahvm-netd` gateway (userspace gVisor netstack): egress is policed (the host, private ranges, and cloud metadata are denied by default), same-owner sandboxes can reach each other, and cross-owner traffic is isolated
...
- **Preview URLs** — `ahvm publish dev -p 3000` → `https://dev-k3m9x2.ahvm.sh`, auto-wake from sleep
- **Session-aware exec** — TTY sessions survive disconnects, scrollback replayed on reattach
- **OCI image support** — `ahvm image pull python:3.12` → use as base for sandboxes
...
- **Single binary** — `ahvm serve` = daemon, `ahvm create` = CLI, `ahvm user` = admin

## Documentation

Full docs live at **[ahvm.sh](https://ahvm.sh)** — that's the canonical reference. The list below is a hand-picked entry point.

| Page | What it covers |
|------|----------------|
| **[Quickstart](https://ahvm.sh/docs/quickstart/)** | Install + create your first sandbox |
...
| **[Decisions & learnings](https://ahvm.sh/docs/under-the-hood/decisions/)** | Why TCP over vsock, why no diff snapshots, the bugs we paid for |
| **[CLI Reference](https://ahvm.sh/docs/reference/cli/)** | All commands and flags |
| **[API Reference](https://ahvm.sh/docs/reference/api/)** | REST/WebSocket endpoints |
| **[Testing](https://ahvm.sh/docs/contributing/testing/)** | 11K lines of tests, zero mocks for VM tests |
