...
// `/proc/self/exe spawn ...`. In production /proc/self/exe is forge's
...
// the argv-verb dispatch in cmd/forge/main.go is never reached, and
...
//     test process with FORGE_SPAWN_HELPER=1 in the env, route directly
...
//     it to set FORGE_SPAWN_HELPER=1 on the subprocess env, so the
...
func TestMain(m *testing.M) {
	if os.Getenv("FORGE_SPAWN_HELPER") == "1" {
	...
	spawnHelperEnv = []string{"FORGE_SPAWN_HELPER=1"}
